...
export const KNOWLEDGE_LIBRARY: KnowledgeItem[] = [
  ...
  course(
    ...
    [
      ['5 个心智模型：LLM / Token / 上下文 / 截止 / 幻觉', 12, '图文'],
      ['擅长与不擅长：AI 起草 + 人工复核', 10, '图文'],
      ['四种用法：对话 / 写文 / 改代码 / 生图', 12, '图文'],
      ['入门安全底线：敏感信息不上传', 10, '清单'],
    ],
  ),
  course(
    {
      ...
      category: '入门起步',
      level: '入门',
      desc: '注册后第一次对话：写问题、追问、新开对话时机、保存结果。',
      source: '自有资料',
      hot: true,
      new: true,
    },
    [
      ...
      ['用默认模型即可', 6, '图文'],
      ['何时追问、何时新开对话', 10, '图文'],
      ['为什么三个字没用：写清背景任务格式', 8, '图文'],
      ['复制模板写出第一条有效消息', 18, '实操'],
      ['第二三轮追问到可粘贴终稿', 15, '实操'],
      ['对话重命名 + 可选附件上传', 12, '步骤'],
      ['报错处理：转圈、额度、答非所问', 12, '清单'],
    ],
  ),
  course(
    {
      ...
      hook: '先网页聊天，再装 Trae；Claude Code / Codex / Cursor 按专篇跟做',
      outcome: '国内网页能聊；Trae/灵码会装；知道 Claude Code、Codex、Cursor 各自去哪装',
      category: '入门起步',
      level: '入门',
      desc: '聊天：豆包/Kimi；编程：Trae/灵码优先，并指引 Claude Code、Codex、Cursor 专篇。',
      source: '自有资料',
      hot: true,
      new: true,
      trackStep: 2,
    ...
    [
      ...
      ['要写代码：Trae.cn / 通义灵码（国内优先）', 15, '步骤'],
      ['编程 Agent：Claude Code / Codex / Cursor 入口与专篇', 15, '图文'],
  ...
  course(
    ...
    [
      ['注册前准备：手机短信、邮箱、密码本', 10, '清单'],
      ['国内：豆包 / 通义手机号登录跟做', 18, '实操'],
      ['海外可选：ChatGPT / Claude 注册与跳过策略', 15, '步骤'],
      ['隐私三分钟：删对话、训练开关、别发密码', 15, '实操'],
      ['数据边界：敏感信息不上传', 10, '清单'],
      ['登录失败处理：验证码、垃圾箱、无痕窗口', 12, '清单'],
    ],
  ),
  course(
    {
      ...
      category: '入门起步',
      level: '入门',
      desc: '7 天路径：心智模型 → 登录 → 提问 → 办公 → 安全 → 选方向。',
      source: '自有资料',
      hot: true,
      new: true,
    },
    [
      ['Day1：心智模型 + 第一次有效对话', 30, '实操'],
      ['Day2：官网登录；（可选）装编程 IDE', 35, '实操'],
      ['Day3：四段式提示词 + 十大翻车自查', 35, '实操'],
      ['Day4：邮件 / 纪要 / PPT / 调研各练一次', 30, '实操'],
      ['Day5：懂 API Key 是什么；敏感信息不上传', 25, '清单'],
      ['Day6：预习工具线或作品线一篇', 30, '图文'],
      ['Day7：复盘：哪些交给 AI、哪些必须人工', 25, '实操'],
    ],
  ),
  ...
  course(
    ...
    [
      ['零代码也能懂：API / API Key 是什么', 12, '图文'],
      ['什么时候才需要 Key（只聊天可跳过）', 8, '清单'],
      ['泄露代价与禁止事项', 12, '清单'],
      ['正确：.env、环境变量、不截图分享', 15, '步骤'],
      ['.gitignore 与扫描 sk-', 12, '实操'],
      ['泄露应急：Revoke → 查账单', 15, '模板'],
      ['Dev/Prod 隔离与最小权限', 10, '图文'],
    ],
  ),
...
export const LEARNING_PATHS: LearningPath[] = [
  {
    id: 'path-basic',
    title: '入门：从零问到会用',
    desc: '心智模型 → 对话 → 登录与装工具（含 Claude Code/Codex）→ 办公 → 安全',
    count: 0,
    courseIds: [
      'ai-what-is',
      'ai-first-chat',
